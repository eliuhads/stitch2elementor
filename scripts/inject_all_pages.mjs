...
// Write PHP to temp
const phpFile = `evg_all_pages_${Date.now()}.php`;
const phpPath = path.join(ROOT, 'temp', phpFile);
...
async function uploadAndExecute() {
  ...
  try {
    ...
    console.log(`📤 Uploading ${phpFile} to web root...`);
    await client.uploadFrom(phpPath, phpFile);
    console.log('✅ Upload complete\n');

    // Execute via HTTP
    const execUrl = `${WP_URL}/${phpFile}?token=${INJECT_SECRET}`;
    console.log(`🚀 Executing: ${execUrl.replace(INJECT_SECRET, '***')}`);
