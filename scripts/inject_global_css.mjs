...
const SITE_URL = process.env.WP_URL || process.env.SITE_URL;
...
try {
  ...
  await client.ensureDir('/wp-content/uploads/evergreen-css/');
  await client.cd('/');
  
  // Upload CSS files
  console.log('📤 Uploading global CSS...');
  await client.uploadFrom(globalCssPath, '/wp-content/uploads/evergreen-css/evergreen-global.css');
  console.log('  ✅ evergreen-global.css uploaded');
  
  if (homepageCss) {
    console.log('📤 Uploading homepage CSS...');
    await client.uploadFrom(hpCssPath, '/wp-content/uploads/evergreen-css/evergreen-homepage-v4.css');
  ...
  await client.cd('/');
  await client.uploadFrom(muPath, '/wp-content/mu-plugins/evergreen-custom-styles.php');
  console.log('  ✅ evergreen-custom-styles.php uploaded');
  
  // Upload cache flush script
  console.log('📤 Uploading cache flush script...');
  await client.uploadFrom(flushPath, '/flush_and_verify.php');
