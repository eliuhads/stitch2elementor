...
const c = new Client();
await c.access({ host: env.FTP_HOST, user: env.FTP_USER, password: env.FTP_PASSWORD, secure: true, secureOptions: { rejectUnauthorized: true } });
...
// Check post-23.css timestamp
const postCssMatch = html.match(/post-23\.css\?ver=(\d+)/);
if (postCssMatch) console.log(`  post-23.css ver: ${postCssMatch[1]} (new if > ${Math.floor(Date.now()/1000)})`);
