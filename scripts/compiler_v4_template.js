/**
 * ============================================================
 * COMPILER V4.1 — HTML to Native Elementor JSON
 * Universal Template — Production-hardened (April 2026)
 * ============================================================
 * 
 * USAGE:
 *   1. Update CONFIG below with YOUR project's BrandBook values
 *   2. Place HTML files in stitch_html/ (or update inputDir)
 *   3. Create page_manifest.json with page list + hero_pairs
 *   4. Run: node compiler_v4_template.js
 *   5. Output: elementor_json/ folder with one .json per page
 *
 * PRODUCTION FIXES (V4.1 over V4):
 *   - bg-[#hex]/opacity Tailwind patterns (rgba conversion)
 *   - Tailwind mobile-first → Elementor desktop-first mapping
 *     (flex-col sm:flex-row → desktop: row, mobile: column)
 *   - space-y-* / space-x-* → flex_gap extraction
 *   - h-screen → min_height: 100vh (not 800px)
 *   - border-l-4 + arbitrary border-[#color] patterns
 *   - w-1/2, w-1/3, md:w-1/2 → Elementor width percentages
 *   - Text-only divs → text-editor widgets (stats banners, etc.)
 *   - Captured bg images from absolute inset-0 positioned divs
 *   - Auto hero image injection from page_manifest.json media
 *   - Default dark background on ALL outer sections
 *   - Smarter tree pruning (keeps containers with structural role)
 *
 * CORRECT ELEMENTOR KEYS (V3 bugs fixed):
 *   - flex_gap (NOT gap)
 *   - flex_align_items (NOT align_items)
 ...
 *   - flex_gap format: {unit,size,sizes,column,row,isLinked}
 * ============================================================
 */
...
// ============================================================
// CONFIG — ⚠️ CUSTOMIZE THIS FOR YOUR PROJECT
// Copy values from your BrandBook / design-system/MASTER.md
// ============================================================
const CONFIG = {
  // ── Paths (relative to this script's location) ──
  inputDir: path.join(__dirname, '..', '..', 'stitch_html'),     // ← Adjust if needed
  outputDir: path.join(__dirname, '..', '..', 'elementor_json'),  // ← Adjust if needed
  manifestPath: path.join(__dirname, '..', '..', 'page_manifest.json'),

  // ── Colors (from your BrandBook — HEX values) ──
  colors: {
    background: '#0B0F1A',         // ← YOUR dark background
    paper: '#F5F3ED',              // ← YOUR light/paper background
    primary: '#368A39',            // ← YOUR primary brand color
    accentCta: '#8FDA3E',          // ← YOUR CTA button color
    primaryDark: '#368A39',        // ← YOUR primary dark variant
    secondary: '#84da7e',          // ← YOUR secondary color
    techBlue: '#28B5E1',           // ← YOUR accent/tech blue
    amber: '#F5A623',              // ← YOUR warning/amber accent
    tertiary: '#90db3f',           // ← YOUR tertiary accent
    onSurface: '#dfe2f2',          // ← Text on dark backgrounds
    onSurfaceVariant: '#bfcab9',   // ← Muted text on dark
    surfaceContainerLowest: '#0a0e19',  // ← Darkest surface
    surfaceContainerLow: '#171b27',
    surfaceContainer: '#1b1f2b',
    surfaceContainerHigh: '#262a36',
    surfaceContainerHighest: '#313441', // ← Lightest dark surface
    white: '#FFFFFF',
    textDark: '#0B0F1A',           // ← Text on light backgrounds
    textMuted: '#4D4D4D',
    surfaceCard: '#161C2C',        // ← Card background
    outlineVariant: '#40493d',     // ← Border/outline color
  },

  // ── Typography (from your BrandBook) ──
  fonts: {
    headline: 'Barlow Condensed',  // ← YOUR headline font
    body: 'Barlow',                // ← YOUR body font
    label: 'Inter',                // ← YOUR label/UI font
    cta: 'Barlow Condensed',       // ← YOUR CTA button font
    tech: 'JetBrains Mono',        // ← YOUR monospace/tech font
  },

  // ── Typography scale (adjust sizes per BrandBook) ──
  typoScale: {
...
async function batchConvert() {
  console.log('╔══════════════════════════════════════════════════╗');
  console.log('║  COMPILER V4.1 — HTML → Native Elementor JSON    ║');
  console.log('║  Production Template (Stitch2Elementor Skill)     ║');
  console.log('║  DOM Walker + Tailwind→Elementor Settings         ║');
  console.log('╚══════════════════════════════════════════════════╝\n');
