...
const phpLines = [
  ...
  "$mu_code .= \"  if (is_page(22) && file_exists(\\$p . 'evergreen-homepage-v4.css'))\\n\";",
  ...
  "  $el_class = '\\\\Elementor\\\\Plugin';",
  "  $el_class::instance()->files_manager->clear_cache();",
  "  $r['elementor'] = 'cleared';",
  "}",
  "",
...
try {
  ...
  for (const f of rootFiles) {
    if (['_purge.php', 'inject_global_css.php', 'inject_all_pages.php', 'evergreen_robust_inject.php'].includes(f.name)) {
      try { await client.remove('/' + f.name); console.log('🗑️ Cleaned: ' + f.name); } catch {}
    }
  }
  
  await client.uploadFrom(phpPath, '_css_inject.php');
...
try {
  ...
  try {
    ...
    if (text.includes('<!doctype') || text.includes('<html')) {
      ...
      await c2.downloadTo(tmpCheck, '/wp-content/mu-plugins/evergreen-custom-styles.php');
